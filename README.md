...
# Amida - 約数の４５度の鏡像予想を使用したプログラム

## 概要

整数０から2000000までの値の持つ約数を新アルゴリズム（約数の４５度の鏡像予想）を使って調べて＊で表示する。  
これは、先に公開した [dstar-dev](https://github.com/chotto2/dstar-dev) を起源とする派生プログラムである。  
当プログラム（以降、amida と呼ぶ）のプロットする＊の模様は、エラトステネスの篩を使って生成した模様と一致する。  
エラトステネスの篩を使用せずに、dstar-dev のREADMEで述べた約数の４５度鏡像予想を使用している。  
そうした意味では、エラトステネスの篩を代替するアルゴリズムとも言える。  
アルゴリズム自体は単純で約数が確定したらドミノ倒しのように４５度傾けると次のドミノパイが確定するのでさらにドミノを連鎖的に倒してゆく。  
この繰り返しで、四則演算なしで全整数の約数が確定してしまう。
ドミノのパイは逆走することもでき、その場合も約数が確定してゆく。  
アルゴリズムとしては魅力的ではあるが、メモリー消費量が大きいという難点がある。  
結果としてのリストは、ファイルサイズが大きいためリポジトリには含めていない。  
**[Releases](https://github.com/chotto2/amida/releases)** から `resultam.txt` をダウンロードして是非とも眺めていって欲しい。  
但し、dstar-dev と見栄えは変わらない。

## 特徴

- 🐳 **Docker対応** - 再現可能なビルド環境
- 📊 **2000000までの約数** - 教育・研究目的に適したサイズ
...
結果ファイル（`resultam.txt`）はファイルサイズが大きいため、リポジトリには含めていない。  
GitHubの **[Releases](https://github.com/chotto2/amida/releases)** ページからダウンロードしてほしい。
...
処理２） n=0 の約数パターンを、n=1 から４５度の角度で展開
...
処理３） n=1 の約数パターンを、n=2 から４５度の角度で展開
...
処理４） n=2 の約数パターンを、n=3 から４５度の角度で展開
...
処理５） n=3 の約数パターンを、n=4 から４５度の角度で展開
...
処理６） n=4 の約数パターンを、n=5 から４５度の角度で展開
...
処理７） n=5 の約数パターンを、n=6 から４５度の角度で展開
...
> **注：**　約数の４５度の鏡像予想に基づくアルゴリズムなので、結果を保証するものではありません。
> 自己責任でお願いします。
...
⚠️ **重要**: このバージョンは教育・研究目的の実装です。整数 2000000 までの約数を扱うため、現代の暗号システム（RSA-4096など）には影響しません。
...
- 論文: arXiv (準備中)
- リポジトリ: <https://github.com/chotto2/amida>
